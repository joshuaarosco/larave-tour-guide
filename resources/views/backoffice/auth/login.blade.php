  ...
  <div class="login-wrapper">
    <div class="bg-pic">
      ...
      <div class="bg-caption pull-bottom sm-pull-bottom text-white p-l-20 m-b-20">
        <a href="{{ route('index') }}"><h2 class="semi-bold text-white">{{config('app.name')}}</h2></a>
        <p class="small"></p>
      </div>
    </div>
    <div class="login-container bg-white">
      <div class="p-l-50 m-l-20 p-r-50 m-r-20 p-t-50 m-t-30 sm-p-l-15 sm-p-r-15 sm-p-t-40">
      <a href="{{ route('index') }}"><h3>{{config()->get('app.name')}}</h3></a>
        ...
        <form action="" method="POST" class="p-t-15" id="form-login" name="form-login" role="form">
          ...
          <div class="row">
            <div class="col-md-6">
              <button class="btn btn-success btn-cons m-t-10 btn-block" type="submit">Sign in</button>
            </div>
            <div class="col-md-6">
              <a href="{{ route('backoffice.auth.sign_up_tourist') }}" class="btn btn-default btn-cons m-t-10 btn-block">Register</a>
            </div>
          </div><br>
        </form>
        <div class="m-t-10">
          <a href="{{ route('backoffice.auth.sign_up') }}" class="normal">Want to be a Tour Guide? Signup now.</a>
        </div>
      </div>
    </div>
  </div>
