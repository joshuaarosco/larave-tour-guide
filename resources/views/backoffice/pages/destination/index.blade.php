            ...
            <div class="row">
                <div class="col-md-8 col-xs-6">
                    ...
                    <div class="pull-right table-btn">
                        ...
                        <a class="btn btn-success btn-create btn-xs md-hidden" href="{{ route('backoffice.destination.create') }}">
                ...
                <div class="col-md-2 sm-hidden">
                    <a class="btn btn-success btn-create btn-block btn-xs table-btn" href="{{ route('backoffice.destination.create') }}">
                        <i class="fa fa-plus"></i>&nbsp;Create
                    </a>
                </div>
            </div>
            ...
            <table class="table table-hover table-condensed table-striped" id="condensedTable">
                <tbody>
                    <tr>
                        ...
                        <!-- <td class="v-align-middle bold" width="30%">Details</td> -->
                        <td class="v-align-middle bold text-right" width="15%">Actions</td>
                    </tr>
                    @forelse($destinations as $index => $destination)
                    <tr>
                        <td>{{$index+1}}</td>
                        <td class="v-align-middle"><img src="{{$destination->thumbnail()}}" alt="" class="prod-img"></td>
                        <td class="v-align-middle">{{$destination->name}}</td>
                        <!-- <td> {!! Str::limit($destination->details, 25) !!} </td> -->
                        <td class="text-right">
                            <!-- <a
                            class="btn btn-default btn-rounded btn-xs"
                            title="view" href="{{route('backoffice.destination.view',$destination->id)}}">
                                <i class="fa fa-eye"></i>
                            </a> -->
                            <a
                            class="btn btn-default btn-rounded btn-xs btn-edit"
                            title="Edit" href="{{ route('backoffice.destination.edit', $destination->id) }}">
                                <i class="fa fa-pencil"></i>
                            </a>
                            <!-- <button
                            class="btn btn-warning btn-rounded btn-xs btn-delete"
                            title="Delete"
                            data-url="{{route('backoffice.destination.delete',$destination->id)}}"
                            data-toggle="modal"
                            data-target="#delete">
